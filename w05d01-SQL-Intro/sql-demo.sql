...
SELECT urls FROM urls WHERE user_id = 1;
...
SELECT urls.long_url, users.email FROM urls JOIN users
...
SELECT users.id, email, COUNT(long_url) FROM users JOIN urls
ON urls.user_id = users.id GROUP BY email, users.id; 

-- Only show users with more than 3 ( 3 inclusive ) urls
SELECT users.id, email, COUNT(long_url) FROM users JOIN urls
ON urls.user_id = users.id GROUP BY email, users.id HAVING COUNT(long_url) >= 3; 
