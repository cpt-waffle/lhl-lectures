...
SELECT users.email, urls.long_url FROM urls RIGHT JOIN  users ON users.id = urls.user_id;
...
SELECT users.id, users.email, COUNT(urls.long_url) FROM urls RIGHT JOIN users ON users.id = urls.user_id
GROUP BY email, users.id;
-- Only show users with more than 3 ( 3 inclusive ) urls
SELECT users.id, users.email, COUNT(urls.long_url) FROM urls RIGHT JOIN users ON users.id = urls.user_id
GROUP BY email, users.id HAVING COUNT(urls.long_url) >= 3